...
class MAGAN(object):
    ...
    def __init__(self,
        ...
        self.xb1 = tf.placeholder(tf.float32,
                                  shape=[None, self.dim_b1],
                                  name='xb1')

        self.xb2 = tf.placeholder(tf.float32,
                                  shape=[None, self.dim_b2],
                                  name='xb2')
    ...
    def _build(self):
        ...
        self.xb1_reconstructed = nameop(self.xb1_reconstructed,
                                        'xb1_reconstructed')
        self.xb2_reconstructed = nameop(self.xb2_reconstructed,
                                        'xb2_reconstructed')
    ...
    def _build_loss_D(self):
        ...
        losses = []
        # the true examples
        losses.append(tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            logits=self.D1_probs_z,
            labels=tf.ones_like(self.D1_probs_z))))

        losses.append(tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            logits=self.D2_probs_z,
            labels=tf.ones_like(self.D2_probs_z))))

        # the generated examples
        losses.append(tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            logits=self.D1_probs_G,
            labels=tf.zeros_like(self.D1_probs_G))))

        losses.append(tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            logits=self.D2_probs_G,
            labels=tf.zeros_like(self.D2_probs_G))))
    ...
    def _build_loss_G(self):
        ...
        losses.append(tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            logits=self.D1_probs_G,
            labels=tf.ones_like(self.D1_probs_G))))
        losses.append(tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            logits=self.D2_probs_G,
            labels=tf.ones_like(self.D2_probs_G))))
        ...
        losses.append(1 * tf.reduce_mean(self.correspondence_loss(self.xb1,
                                                                  self.Gb2)))
        losses.append(1 * tf.reduce_mean(self.correspondence_loss(self.xb2,
                                                                  self.Gb1)))
    ...
    def _build_optimization(self):
        ...
        with tf.control_dependencies(G_update_ops):
            ...
            self.train_op_G = optG.minimize(self.loss_G,
                                            var_list=Gvars,
                                            name='train_op_G')

        with tf.control_dependencies(D_update_ops):
            optD = tf.train.AdamOptimizer(self.lr, beta1=.5, beta2=.99)
            self.train_op_D = optD.minimize(self.loss_D,
                                            var_list=Dvars,
                                            name='train_op_D')
    ...
    def get_layer(self, xb1, xb2, name):
        """Get a layer of the network by name for the
        entire datasets given in xb1 and xb2."""
    ...
    def get_loss_names(self):
        ...
        tnf_losses = tf.get_collection('losses')
        losses = [tns.name[:-2].replace('loss_', '').split('/')[-1] for tns in tnf_losses]
